...
import { topPlayers } from "@/lib/cricket-data";
...
export function PlayersSection() {
  ...
  const orangeCap = [...topPlayers].sort((a, b) => b.runs - a.runs)[0];
  const purpleCap = [...topPlayers].sort((a, b) => b.wickets - a.wickets)[0];
    ...
    <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-orange-400">Orange Cap</h3>
              <p className="text-xs text-muted-foreground">Highest Run Scorer</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <Avatar className="w-14 h-14 border-2 border-orange-500/50">
              <AvatarFallback
                ...
                style={{ backgroundColor: orangeCap.teamColor }}
              >
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-purple-400">Purple Cap</h3>
              <p className="text-xs text-muted-foreground">Highest Wicket Taker</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <Avatar className="w-14 h-14 border-2 border-purple-500/50">
              <AvatarFallback
                className="text-lg font-bold text-white"
                style={{ backgroundColor: purpleCap.teamColor }}
              >
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as PlayerTab)} className="mb-6">
        <TabsList className="bg-card/80 border border-border/50">
          ...
          <TabsTrigger value="batsmen" className="text-xs sm:text-sm">
            <Swords className="w-3 h-3 mr-1" />
            Batsmen
          </TabsTrigger>
          <TabsTrigger value="bowlers" className="text-xs sm:text-sm">
            <Target className="w-3 h-3 mr-1" />
            Bowlers
          </TabsTrigger>
          <TabsTrigger value="allrounders" className="text-xs sm:text-sm">
        ...
        {filteredPlayers.map((player, index) => (
          ...
          >
            <div className="flex items-start gap-4 mb-4">
              <Avatar className="w-14 h-14 border-2 shrink-0 group-hover:scale-105 transition-transform"
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge
                    ...
                    style={{
                      ...
                      borderColor: `${player.teamColor}30`,
                    }}
                  >
                    {player.teamShort}
                  </Badge>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-card/50 rounded-lg p-3">
                ...
                <p className="text-lg font-bold text-green-400">{player.runs}</p>
              ...
              <div className="bg-card/50 rounded-lg p-3">
                ...
                <p className="text-lg font-bold text-purple-400">{player.wickets}</p>
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                Best: {player.bestBatting} | {player.wickets > 0 ? `${player.wickets} wickets` : "N/A"}
              </span>
            </div>
          </motion.div>
        ))}
